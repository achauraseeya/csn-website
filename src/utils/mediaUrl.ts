...
export function getDriveFileViewUrl(urlOrId: string): string {
...
/**
 * Formats numbers so that English digits (0-9) are displayed in English mode,
 * and Devanagari/Nepali digits (०-९) are displayed in Nepali mode.
 */
export function formatNumber(input: string | number | undefined | null, lang: 'en' | 'ne'): string {
  ...
  if (lang === 'en') {
    ...
    return str.replace(/[०-९]/g, match => neToEn[match] || match);
  } else {
    const enToNe: Record<string, string> = {
      '0': '०', '1': '१', '2': '२', '3': '३', '4': '४',
      '5': '५', '6': '६', '7': '७', '8': '८', '9': '९'
    };
    return str.replace(/[0-9]/g, match => enToNe[match] || match);
  }
}
