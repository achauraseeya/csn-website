...
export default function MatrimonialSection({
  ...
  const [interestModalProfile, setInterestModalProfile] = useState<MatrimonialProfile | null>(null);

  // Express Interest Form State
  const [interestSenderName, setInterestSenderName] = useState('');
  const [interestSenderPhone, setInterestSenderPhone] = useState('');
  const [interestSenderEmail, setInterestSenderEmail] = useState('');
  const [interestSenderAddress, setInterestSenderAddress] = useState('');
  const [interestProposingCandidate, setInterestProposingCandidate] = useState('');
  ...
  // Helper function to get or generate unique ID for candidate
  const getCandidateUniqueId = (profile: MatrimonialProfile, index: number = 0) => {
    if (profile.uniqueId) return profile.uniqueId;
    const num = 101 + index;
    return `CSN-MAT-${num}`;
  };

  // Back button history state handler for mobile pop-ups
  React.useEffect(() => {
    const handlePopState = () => {
      if (selectedProfile) {
        setSelectedProfile(null);
      } else if (interestModalProfile) {
  ...
  }, [selectedProfile, interestModalProfile, isRegisterModalOpen]);

  const refreshCustomFields = () => {
    setCustomFields(getCustomFormFields('matrimonial'));
  };

  const openProfileDetails = (profile: MatrimonialProfile) => {
    window.history.pushState({ modal: 'matrimonial-detail' }, '');
    setSelectedProfile(profile);
  };

  const openRegisterModal = () => {
    window.history.pushState({ modal: 'matrimonial-register' }, '');
    setIsRegisterModalOpen(true);
  };

  const openInterestModal = (profile: MatrimonialProfile) => {
    setInterestModalProfile(profile);
    setInterestSubmitted(false);
    window.history.pushState({ modal: 'matrimonial-interest' }, '');
  };

  const handleSendInterestSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!interestModalProfile) return;
    if (!interestSenderName || !interestSenderPhone) {
      alert(lang === 'en' ? 'Please fill in all required fields.' : 'कृपया आवश्यक विवरणहरू भरूहोस्।');
      return;
    }

    const uniqueId = getCandidateUniqueId(interestModalProfile, profiles.indexOf(interestModalProfile));

    const formData = new FormData();
    formData.append('_subject', `[Express Interest] Candidate ID: ${uniqueId} - ${interestModalProfile.fullName}`);
    formData.append('_template', 'table');
    formData.append('_captcha', 'false');
    formData.append('Candidate Unique ID', uniqueId);
    formData.append('Candidate Full Name', interestModalProfile.fullName);
    formData.append('Candidate Looking For', interestModalProfile.lookingFor === 'groom' ? 'Groom (वर)' : 'Bride (वधू)');
    formData.append('Candidate Guardian Phone', interestModalProfile.guardianPhone);
    formData.append('Sender Family Name', interestSenderName);
    formData.append('Sender Phone / WhatsApp', interestSenderPhone);
    formData.append('Sender Email', interestSenderEmail || 'Not provided');
    formData.append('Sender Address / Location', interestSenderAddress || 'Not provided');
    formData.append('Details of Proposing Candidate', interestProposingCandidate || 'Will share directly during phone discussion');
    formData.append('Personal Message', interestMessage || 'Interested in family discussion for marriage match.');
    ...
    onTrackAction(`Expressed Interest for candidate ${uniqueId} - ${interestModalProfile.fullName}`);
    setInterestSentId(interestModalProfile.id);
    setInterestSubmitted(true);
    setTimeout(() => {
      setInterestSubmitted(false);
      setInterestModalProfile(null);
      setInterestSenderName('');
      setInterestSenderPhone('');
      setInterestSenderEmail('');
      setInterestSenderAddress('');
      setInterestProposingCandidate('');
      setInterestMessage('');
    }, 2500);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            >
              ...
              <div className="relative h-60 bg-teal-900 overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 max-w-[90%]">
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider text-white ${profile.lookingFor === 'groom' ? 'bg-indigo-600' : 'bg-rose-600'}`}>
                    {profile.lookingFor === 'groom' ? 'वर (Groom)' : 'वधू (Bride)'}
                  </span>
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-slate-900/90 text-amber-300 border border-amber-400/30 flex items-center gap-1 shadow-sm font-mono">
                    ID: {getCandidateUniqueId(profile, filteredProfiles.indexOf(profile))}
                  </span>
                  {profile.status === 'approved' && (
                    <span className="px-2 py-1 rounded-full text-[10px] font-black bg-emerald-500 text-white flex items-center gap-1 shadow-sm">
              ...
              <div className="p-4 bg-teal-50/50 dark:bg-slate-800/50 border-t border-teal-100 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <button
                  onClick={() => openInterestModal(profile)}
                ...
                >
                  ...
                  {interestSentId === profile.id ? (lang === 'en' ? 'Interest Sent!' : 'पठाइयो!') : (lang === 'en' ? 'Express Interest' : 'इच्छा व्यक्त गर्नुहोस्')}
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl max-w-2xl w-full border border-teal-100 dark:border-slate-800 shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col animate-in zoom-in-95 duration-200">
            <div className="relative h-48 sm:h-64 bg-slate-950 shrink-0">
              ...
              <div className="absolute bottom-4 left-6 text-white space-y-1">
                ...
                <h2 className="text-2xl sm:text-3xl font-black">{formatNumber(selectedProfile.fullName, lang)}</h2>
            ...
            <div className="p-4 sm:p-8 space-y-4 sm:space-y-6 text-xs text-gray-700 dark:text-gray-300 font-medium overflow-y-auto flex-1">
      ...
      {isRegisterModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm p-2 sm:p-4 md:p-6 overflow-y-auto flex items-start sm:items-center justify-center min-h-screen">
          <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl max-w-2xl w-full border border-teal-100 dark:border-slate-800 shadow-2xl p-4 sm:p-8 space-y-4 sm:space-y-6 my-auto max-h-[92vh] flex flex-col animate-in zoom-in-95 duration-200">
      ...
      {interestModalProfile && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm p-3 sm:p-6 overflow-y-auto flex items-center justify-center min-h-screen">
          <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl max-w-lg w-full border border-rose-200 dark:border-slate-800 shadow-2xl p-5 sm:p-7 space-y-5 my-auto animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start border-b pb-3 dark:border-slate-800">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 text-[10px] font-black uppercase">
                  <Heart className="w-3 h-3 fill-rose-600 text-rose-600" />
                  {lang === 'en' ? 'Marriage Proposal Interest Form' : 'वैवाहिक प्रस्ताव इच्छा फारम'}
                </div>
                <h3 className="text-lg sm:text-xl font-black text-teal-950 dark:text-teal-100">
                  {lang === 'en' ? 'Express Interest for:' : 'इच्छा व्यक्त गर्नुहोस्:'} {interestModalProfile.fullName}
                </h3>
                <p className="text-xs text-rose-600 dark:text-rose-400 font-bold font-mono">
                  Candidate Unique ID: {getCandidateUniqueId(interestModalProfile, profiles.indexOf(interestModalProfile))}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setInterestModalProfile(null)}
                className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {interestSubmitted ? (
              <div className="p-6 text-center space-y-3 bg-emerald-50 dark:bg-emerald-950/40 rounded-2xl border border-emerald-300">
                ...
                <h4 className="text-xl font-black text-emerald-950 dark:text-emerald-100">
                  {lang === 'en' ? 'Interest Form Sent Successfully!' : 'इच्छा फारम सफलतापुर्वक पठाइयो!'}
                </h4>
                <p className="text-xs text-emerald-800 dark:text-emerald-300 font-medium">
                  {lang === 'en'
                    ? 'Your interest details & Unique ID have been submitted to csnepalwebsite@gmail.com. Our Chaurasiya Samaj team will connect both families.'
                    : 'तपाईंको वैवाहिक रुचि विवरण csnepalwebsite@gmail.com मा प्राप्त भएको छ। समाजले दुवै परिवारलाई सम्पर्क गराउनेछ।'}
                </p>
              </div>
            ) : (
              <form onSubmit={handleSendInterestSubmit} className="space-y-3.5 text-xs font-semibold">
                <div>
                  <label className="block text-gray-700 dark:text-gray-300 mb-1">
                    Your / Family Full Name *
                  </label>
                  <input
                    type="text"
                    required
                    value={interestSenderName}
                    onChange={e => setInterestSenderName(e.target.value)}
                    placeholder="e.g. Shyam Sundar Chaurasiya"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-gray-900 dark:text-white"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="tel"
                      required
                      value={interestSenderPhone}
                      onChange={e => setInterestSenderPhone(e.target.value)}
                      placeholder="+977-9800000000"
                      className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-gray-900 dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="email"
                      value={interestSenderEmail}
                      onChange={e => setInterestSenderEmail(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={interestSenderAddress}
                    onChange={e => setInterestSenderAddress(e.target.value)}
                ...
                <div>
                  <label className="block text-gray-700 dark:text-gray-300 mb-1">
                    Details of Proposing Candidate (Name, Age, Education, Profession)
                  </label>
                  <textarea
                    rows={2}
                    value={interestProposingCandidate}
                    onChange={e => setInterestProposingCandidate(e.target.value)}
                    placeholder="e.g. Proposing for my son Rahul (Age 26, Engineer, Birgunj)"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-gray-900 dark:text-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. We would like to connect with candidate guardian for further talk..."
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-gray-900 dark:text-white"
                  />
                </div>
                ...
                <div className="flex justify-end gap-2.5 pt-3 border-t dark:border-slate-800">
                  <button
                    type="button"
                    onClick={() => setInterestModalProfile(null)}
                    className="px-4 py-2 rounded-xl border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-gray-300 font-bold"
                  >
                    {lang === 'en' ? 'Cancel' : 'रद्द गर्नुहोस्'}
                  </button>
                  <button
                    type="submit"
                    className="px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold flex items-center gap-1.5 shadow-md cursor-pointer"
                  >
                    <Heart className="w-4 h-4 fill-white" />
                    {lang === 'en' ? 'Send Interest Proposal' : 'प्रस्ताव पठाउनुहोस्'}
                  </button>
                </div>
              </form>
            )}
